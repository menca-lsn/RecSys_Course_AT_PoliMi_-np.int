...
cdef class BPRIterator:
    ...
    def __next__(self):
        ...
        if self.n_negatives_per_positive == 1:
            return torch.from_numpy(np.array(self.batch_user[:i_batch+1], dtype=int64)),\
                   torch.from_numpy(np.array(self.batch_positive_item[:i_batch+1], dtype=int64)), \
                   torch.from_numpy(np.array(self.batch_negative_item[:i_batch+1], dtype=int64))
        else:
            return torch.from_numpy(np.array(self.batch_user[:i_batch+1], dtype=int64)),\
                   torch.from_numpy(np.array(self.batch_positive_item[:i_batch+1], dtype=int64)), \
                   torch.from_numpy(np.array(self.batch_multiple_negative_item[:i_batch+1,:], dtype=int64))
